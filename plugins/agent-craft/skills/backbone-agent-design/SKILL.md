---
name: backbone-agent-design
description: "Method for making a SMALL/CHEAP LLM behave like a much larger one by offloading intelligence into a BACKBONE of facilities instead of model weights. Use when designing, auditing, or improving any agent/assistant built on a small or budget model (4B/7B/8B, or any model you can't afford to scale) — especially when the model hallucinates, loses the thread, ignores instructions, or feels dumber than it should. Gives a quality formula, a 7-layer / 17-facility catalog, the 3-point output discipline, and a biggest-impact-first rollout. Reusable across projects, moats, and other agentic systems."
metadata:
  type: methodology
---
...
- **Experience memory needs a trust loop** (TAME, arXiv 2602.03224 — production-verified): any memory that GROWS from experience (strategy banks, learned lessons) degrades the agent unless each item earns trust. Per item: `uses` (bumped on actual injection, not retrieval), `helped`/`hurt` (owner rating on injected turns only — a turn with NO feedback is NOT negative signal), Laplace trust `(helped+1)/(helped+hurt+2)`, auto-demote when repeatedly harmful (e.g. hurt≥3 & trust<0.25). Trust re-ranks; it must NEVER relax the injection threshold (keep the base-score gate or high-trust items break your anti-spam invariant).
...
- **Act-then-Speak decision gate** (production-verified on a 4B): small models PROMISE actions instead of doing them ("I'll read that URL now" ×20 turns, zero calls) because prose is always the higher-probability continuation of a prompt-defined tool format. Fix structurally: at the action-decision step, constrain decoding to a schema whose ONLY productions are `{action:"tool", tool:<ENUM of the actual available names>, args}` or `{action:"final"}` — a promise is not in the grammar. Two measured lessons: (a) the tool field MUST be an enum — a free string got call-syntax smuggled in (`web_read(url=...)`), enum+2-few-shot moved 4/6→6/6 at temp 0; (b) don't ask the gate for good args — the gate CHOOSES, deterministic code FILLS (URL regex from message, protocol canonicalization). Roll out shadow-first: log gate-decision vs actual behavior, wire `on` only after agreement calibrates.
...
2. **Audit against the 17-facility menu.** Mark each EXISTS / PARTIAL / PLANNED against the real code. This is your map (see MiganCore doc 64 for a worked example).
