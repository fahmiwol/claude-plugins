---
name: local-gpu-bridge
description: "Let an always-on CPU server (VPS/self-hosted brain) BORROW a consumer laptop/desktop GPU over a reverse-SSH tunnel — no port-forwarding, no GPU rent, antifragile failover (GPU can only ADD speed, never lose work). Use when a self-hosted AI system needs occasional heavy inference (extraction, summarization, teacher-gen, training) and the owner has a gaming/consumer GPU at home; or when shipping 'use your own GPU' to end users. Encodes the real pitfalls: custom docker bridge subnets, UFW container->host blocks, GatewayPorts, RAM-OOM 500s."
metadata:
  type: methodology
---

# Local GPU Bridge — the "Steam Remote Play" pattern for self-hosted AI

> Origin: Fahmi Ghani / MiganCore F-175 (2026-07). Founder insight: "Chrome and Steam
...
## v2 — shipping it: restricted tunnel user + one-script installer (F-179, proven E2E)
...
## v2 — beyond text: local IMAGE-GEN over the same tunnel (F-180, proven E2E)
